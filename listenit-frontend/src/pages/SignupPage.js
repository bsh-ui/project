...
import '../styles/Signup.css'; // SignupPage 전용 CSS (이 파일도 src/styles 에 있어야 함)
...
function SignupPage() {
    ...
    // 에러 메시지 표시 헬퍼 함수 (컴포넌트 내부로 이동)
    const showError = (setter, message) => {
        setter(message);
    };

    // 에러 메시지 숨기기 헬퍼 함수 (컴포넌트 내부로 이동)
    ...
    const checkDuplicate = async (field, value, errorSetter) => {
        if (!value.trim() || !validateField(field, value)) {
            // 값이 비어있거나 클라이언트 측 유효성 검사 실패 시 중복 확인 건너뛰고 에러 표시
            return;
        }
    ...
    // ⭐⭐⭐ 누락된 handle...Blur 함수들 정의 ⭐⭐⭐
    const handleUsernameBlur = () => {
        checkDuplicate('username', username, setUsernameError);
    };

    const handleEmailBlur = () => {
        checkDuplicate('email', email, setEmailError);
    };

    const handlePasswordBlur = () => {
        validateField('password', password); // 비밀번호는 중복 확인이 아닌 유효성 검사만
    };

    const handleNicknameBlur = () => {
        checkDuplicate('nickname', nickname, setNicknameError);
    };
    // ⭐⭐⭐ 누락된 handle...Blur 함수들 정의 끝 ⭐⭐⭐
