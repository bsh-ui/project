    ...
    public void logLoginSuccess(Long userId, String username, String ipAddress) {
        try {
            UserActivityLog logEntry = new UserActivityLog(
                userId,
                "LOGIN", // 활동 타입 (예: LOGIN, LOGOUT, PROFILE_UPDATE 등)
                ipAddress,
                "User " + username + " logged in successfully." // 상세 정보
            );
            userActivityLogRepository.save(logEntry);
            log.info("User activity logged: userId={}, type={}, details={}", userId, "LOGIN", "User " + username + " logged in successfully.");
        } catch (Exception e) {
            log.error("Failed to log user activity for userId {}: {}", userId, e.getMessage(), e);
        }
    }
